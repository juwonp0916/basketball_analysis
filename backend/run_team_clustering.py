...
# Add score_detection to path so we can import team_detector
...
def main():
    ...
    # Load config to get YOLO weights and classes
    ...
    for video_path in video_files:
        ...
        # Inference dims - matching StreamingShotDetector logic
        ...
        while True:
            ...
            if not team_detector.is_configured:
                # Accumulate features and attempt to calibrate colors (Phase 1)
                team_detector.auto_calibrate(frame, model, class_names, inference_dims, device)
                
                # Draw calibration status on the video
                cv2.putText(frame, "Calibrating Teams...", (30, 50), 
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 165, 255), 2)
            else:
                # Every N frames, perform silent recheck to fix color drift
                if frame_idx % 60 == 0:
                ...
                # Run YOLO inference explicitly to draw bounded boxes
                ...
                for r in results:
                    for box in r.boxes:
                        ...
                        # Rescale coordinates to the original frame
                        ...
                        # Note: tracking ID is skipped here for simplicity, classification operates frame-by-frame
                        ...
                        if team_id is not None:
                            ...
                            box_color = (128, 128, 128) # Gray for uncertain bounding boxes
                            label = "Unknown"
                ...
                # Draw team colors legend at the top
