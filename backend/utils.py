...
def detect_score(ball_pos, hoop_pos, last_pos, debug=False):
    if len(ball_pos) < 2:
        return False
    ...
    if debug:
        print(f"  [detect_score] Ball: ({x1},{y1}) -> ({x2},{y2})")
        print(f"  [detect_score] Hoop box: x[{hoop_l:.0f},{hoop_r:.0f}] y[{hoop_t:.0f},{hoop_b:.0f}]")

    if hoop_l < x2 < hoop_r and hoop_t < y2 < hoop_b:
        if debug:
            print(f"  [detect_score] Ball IS inside hoop box -> SCORE!")
        return True

    if x1 == x2 or y1 == y2:
        if debug:
            print(f"  [detect_score] No movement (x1==x2 or y1==y2)")
        return False

    # ball has to be moving down
    if y1 > y2:
        if debug:
            print(f"  [detect_score] Ball moving UP (y1={y1} > y2={y2})")
        return False
    ...
    if (hoop_l < x1 < hoop_r) and (hoop_l < x2 < hoop_r) and (hoop_y1 < y1 < hoop_y2) and (hoop_y1 < y2 < hoop_y2):
        if debug:
            print(f"  [detect_score] Both points inside hoop -> SCORE!")
        return True
    ...
    if y1 < hoop_y_mid and y2 > hoop_y_mid:
        ...
        if debug:
            print(f"  [detect_score] Linear interpolation: hoop_y_mid={hoop_y_mid}, intersect_x={intersect_x:.0f}")
            print(f"  [detect_score] Hoop x range: [{hoop_l:.0f}, {hoop_r:.0f}]")

        result = hoop_l < intersect_x < hoop_r
        if debug:
            print(f"  [detect_score] Trajectory crosses hoop: {result}")
        return result

    if debug:
        print(f"  [detect_score] No linear interpolation condition met (y1={y1}, hoop_y_mid={hoop_y_mid}, y2={y2})")
    return False
